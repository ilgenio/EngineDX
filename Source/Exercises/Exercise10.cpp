...
bool Exercise10::init() 
{
    ...
    if(ok)
    {
        ...
        imguiPass = std::make_unique<ImGuiPass>(d3d12->getDevice(), d3d12->getHWnd(), debugTableDesc.getCPUHandle(0), debugTableDesc.getGPUHandle(0));
        ...
        spotLight.Lp = Vector3(-1.6f, 2.5f, 1.5f);
        spotLight.Ld = Vector3(0.32f, -0.682f, -0.657f);
        spotLight.inner = 0.93f;
        spotLight.outer = 0.88f;
        spotLight.sqRadius = 25.0f;
        spotLight.intensity = 25.0f;
...
void Exercise10::ddDirectional(Directional &dirLight, float distance, float size)
{
...
void Exercise10::imGuiCommands()
{
    ...
    for (BasicMaterial& material : model->getMaterials())
    {
        if (material.getMaterialType() == BasicMaterial::PBR_PHONG)
        {
            ...
            {
                MetallicRoughnessMaterialData matData = material.getMetallicRoughnessMaterial();
                if (ImGui::ColorEdit3("Base Colour", reinterpret_cast<float*>(&matData.baseColour)))
                {
                    material.setMetallicRoughnessMaterial(matData);
                }

                bool hasTexture = matData.hasBaseColourTex;
                if (ImGui::Checkbox("Use Texture", &hasTexture))
                {
                    matData.hasBaseColourTex = hasTexture;
                    material.setMetallicRoughnessMaterial(matData);
                }
      
                if (ImGui::DragFloat("Metallic Factor", reinterpret_cast<float*>(&matData.metallicFactor)))
                {
                    material.setMetallicRoughnessMaterial(matData);
                }

                if (ImGui::DragFloat("Roughness factor", &matData.roughnessFactor))
                {
                    material.setMetallicRoughnessMaterial(matData);
                }
            }
...
void Exercise10::renderToTexture(ID3D12GraphicsCommandList* commandList)
{
    ModuleD3D12* d3d12 = app->getD3D12();
    ModuleCamera* camera = app->getCamera();
    ModuleShaderDescriptors* descriptors = app->getShaderDescriptors();
    ...
    commandList->SetGraphicsRootShaderResourceView(3, ringBuffer->alloc(&dirLight, std::size(dirLight)));
    commandList->SetGraphicsRootShaderResourceView(4, ringBuffer->alloc(&pointLight));
    commandList->SetGraphicsRootShaderResourceView(5, ringBuffer->alloc(&spotLight));
    ...
    for (const BasicMesh& mesh : model->getMeshes())
    {
        if (UINT(mesh.getMaterialIndex()) < model->getNumMaterials())
        {
            ...
            const ShaderTableDesc& textureDesc = material.getTexturesTableDesc();
...
bool Exercise10::createRootSignature()
{
    ...
    tableRanges.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 3);
...
bool Exercise10::createPSO()
{
    ...
    auto dataVS = DX::ReadData(L"Exercise10VS.cso");
    auto dataPS = DX::ReadData(L"Exercise10PS.cso");
