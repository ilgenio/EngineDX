...
ComPtr<ID3D12Resource> ModuleResources::createUploadBuffer(size_t size, const char* name)
{
    ModuleD3D12* d3d12 = app->getD3D12();
...
{
    ComPtr<ID3D12Resource> buffer = createUploadBuffer(size, name);
...
ComPtr<ID3D12Resource> ModuleResources::createDefaultBuffer(size_t size, D3D12_RESOURCE_FLAGS flags, const char* name)
{
...
{
    ComPtr<ID3D12Resource> buffer = createDefaultBuffer(size, D3D12_RESOURCE_FLAG_NONE, name);
    ComPtr<ID3D12Resource> upload = createUploadBuffer(size, nullptr);
...
ComPtr<ID3D12Resource> ModuleResources::createTextureFromImage(const ScratchImage& image, const char* name)
{
    ...
    if (metaData.dimension == TEX_DIMENSION_TEXTURE2D)
    {
        ...
        if (ok)
        {
            ...
            upload = createUploadBuffer(GetRequiredIntermediateSize(texture.Get(), 0, UINT(image.GetImageCount())));
            ok = upload != nullptr;
        }
